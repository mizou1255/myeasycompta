      ...
      <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 w-[90vw] max-w-2xl shadow-2xl border border-slate-100 dark:border-slate-800 text-left relative overflow-hidden">
        ...
        <form v-else @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="flex items-center gap-4 px-1">
                <button
                    type="button"
                    @click="editedItem.is_optional = editedItem.is_optional ? 0 : 1"
                    :class="[
                        ...
                        editedItem.is_optional ? 'bg-amber-400' : 'bg-slate-200 dark:bg-slate-700'
                    ]"
                >
                    <span :class="['pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out', editedItem.is_optional ? 'translate-x-5' : 'translate-x-0']"></span>
                </button>
                <div>
                    <p class="text-sm font-black text-slate-700 dark:text-slate-200">{{ editedItem.is_optional ? 'Élément optionnel' : 'Élément obligatoire' }}</p>
                    <p class="text-[11px] text-slate-400">{{ editedItem.is_optional ? 'Le client pourra accepter ou refuser cet élément' : 'Inclus automatiquement dans le devis' }}</p>
                </div>
            </div>
...
const submitForm = async () => {
    ...
    try {
        const response = await axios.put(
            `/wp-json/my-easy-compta/v1/quotes/edit-item/${editedItem.value.id}`, 
            editedItem.value,
